...
var _helperFunctions = require("./js/helperFunctions");
// {commonjs import}
...
// events handlers
leftSideForm.addEventListener("submit", _helperFunctions.renderLeftSideMovie);
rightSideForm.addEventListener("submit", _helperFunctions.renderRightSideMovie); // {es6 named export}

},{"./js/helperFunctions":"7JMdY"}],"7JMdY":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "calcMovieStats", ()=>calcMovieStats
);
parcelHelpers.export(exports, "createGenreLabels", ()=>createGenreLabels
);
parcelHelpers.export(exports, "createActorLabels", ()=>createActorLabels
);
parcelHelpers.export(exports, "fetchMovieByType", ()=>fetchMovieByType
);
parcelHelpers.export(exports, "renderLeftSideMovie", ()=>renderLeftSideMovie
);
parcelHelpers.export(exports, "renderRightSideMovie", ()=>renderRightSideMovie
);
var _renderMovie = require("./renderMovie");
var _renderMovieDefault = parcelHelpers.interopDefault(_renderMovie);
...
const fetchMovieByType = async function(searchTerm, opponentSide) {
    try {
        if (!searchTerm) showError("Input cannot be empty ⛔️", "negative");
        // search based on input value
        const movieData = await _requestDefault.default(searchTerm);
        // show movie details
        _renderMovieDefault.default(movieData, opponentSide);
        // check if {left or right side} and set {left and right} side movie value
        if (opponentSide === leftSideMovieOpponent) leftSideMovie = movieData;
        else rightSideMovie = movieData;
        // check if left and right movies are defined
        if (leftSideMovie && rightSideMovie) calcMovieStats();
        // clear input
...
const calcMovieStats = function() {
    const leftSideStats = document.querySelectorAll(".left-side-box-stats-summary .stats");
    const rightSideStats = document.querySelectorAll(".right-side-box-stats-summary .stats");
    const leftSideRatingsStats = document.querySelectorAll(".left-side-box-rating .rating-stats");
    const rightSideRatingsStats = document.querySelectorAll(".right-side-box-rating .rating-stats");
    calcStats({
        leftSideStats,
        rightSideStats
    });
    calcRatingStats({
        leftSideRatingsStats,
        rightSideRatingsStats
    });
};
...
const renderMovie = function(data, element) {
    try {
        ...
        const awards = data.Awards.split(" ").reduce((prev, word)=>{
            ...
            if (data?.BoxOffice) dollarValue = parseInt(data.BoxOffice.replaceAll("$", "").replaceAll(",", ""));
            // {check if NAN}
  ...
  `;
        ...
        // create genre labels
        _helperFunctions.createGenreLabels(data.Genre, `${element.className}`);
        // create actor labels
        _helperFunctions.createActorLabels(data.Actors, `${element.className}-actors`);
    } catch (error) {
...
},{"./../utils":"5By4s"}]},["d5kvp","igcvL"], "igcvL", "parcelRequire85d4")
